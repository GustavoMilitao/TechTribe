	...
	>
		<div class="flex justify-center flex-col">
			<GreenButton
				...
				}}>{$t('quiztivity.editor.shares.add_new_share')}</GreenButton
			>
			{#if add_shares_open}
				<form
				...
				>
					...
					<GreenButton type="submit" disabled={!selected_date && !never_expires_checked}
				...
				<div class="grid grid-cols-4 w-full gap-2" in:fade={{ duration: 50 }}>
					...
					<div class="w-full mx-auto">
						<GreenButton
							...
							on:click={() => {
								navigator.share({
									title: 'Quiztivity on TechTribe',
									text: 'Play this Quiztivity now on TechTribe!',
