		...
		<div class="border-[#B07156] border-2 rounded">
			...
			<div class="grid grid-cols-2 py-2">
				<div class="px-2 flex flex-col gap-2">
					...
					<GreenButton>{$t('quiztivity.memory.editor.upload_image')}</GreenButton>
				</div>
				<div class="px-2 flex flex-col gap-2">
					<textarea
						type="text"
			...
			<div class="flex justify-center p-2">
				<GreenButton on:click={add_card}
					>{$t('quiztivity.memory.editor.add_pair')}</GreenButton
				>
			</div>
		</div>
			...
			>
				...
				<div class="grid grid-cols-2 p-2 gap-2">
					<GreenButton
						...
						disabled={i === 0}>{$t('quiztivity.editor.move_left')}</GreenButton
					>
					<GreenButton
						on:click={() => {
							move_card_right(i);
