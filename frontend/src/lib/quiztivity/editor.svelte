	...
	<div>
			...
			{#if data.id}
				<div class="mr-auto w-fit pl-2">
					<GreenButton
						...
						}}>{$t('quiztivity.editor.open_shares_menu')}</GreenButton
					>
				</div>
			{:else}
				<span />
			...
			<div class="self-end pr-2 w-full">
				<div class="ml-auto w-fit">
					<GreenButton
						...
						disabled={!data.title}>{$t('words.save')}</GreenButton
					>
				</div>
			</div>
		</div>
		<div class="flex flex-row gap-2 w-full p-2">
			<GreenButton
				on:click={() => {
					selected_type = null;
				}}>{$t('quiztivity.editor.add_new')}</GreenButton
			>
			<GreenButton on:click={delete_slide} disabled={selected_slide === null}
				>{$t('quiztivity.editor.delete')}</GreenButton
			>
			<GreenButton
				on:click={move_slide_left}
				disabled={selected_slide === null || selected_slide === 0}
				>{$t('quiztivity.editor.move_left')}</GreenButton
			>
			<GreenButton
				on:click={move_slide_right}
				disabled={selected_slide === null || selected_slide === data.pages.length - 1}
				>{$t('quiztivity.editor.move_right')}</GreenButton
			>
		</div>
