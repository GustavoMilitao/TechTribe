...
<div class="w-screen h-screen m-auto flex flex-col dark:bg-gray-700 p-2 bg-white">
	...
	<div class="flex relative my-2">
		<div class="w-full">
			<GreenButton disabled={status !== Status.CompressDone} on:click={upload_video}>
				{$t('words.upload')}
				{file_size_in_mi ? `(${file_size_in_mi.toFixed(2)} Mi)` : ''}</GreenButton
			>
		</div>
		<span
	...
	<div class="flex justify-center">
		<GreenButton on:click={compress_video} disabled={status !== Status.Idle}
