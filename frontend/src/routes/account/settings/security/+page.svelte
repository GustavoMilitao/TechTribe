	...
	<div class="grid grid-rows-2 h-screen">
		<div class="grid grid-cols-2 h-full border-b-2 border-black">
			<div class="h-full w-full border-r-2 border-black">
				...
				<div class="flex h-full w-full justify-center">
					<div class="m-auto">
						<GreenButton on:click={get_backup_code}
							>{$t('security_settings.get_backup_code')}</GreenButton
						>
					</div>
				</div>
			</div>
		...
		<div class="grid grid-cols-2 h-full">
			<div class="h-full w-full flex flex-col border-r-2 border-black">
				...
				<div class="flex justify-center">
					<div class="m-auto">
						<GreenButton on:click={add_security_key}
							>{$t('security_settings.add_security_key')}</GreenButton
						>
					</div>
				</div>
				<div class="flex justify-center">
			...
			<div class="h-full w-full flex flex-col">
				...
				<div class="flex justify-center">
					<div class="m-auto">
						{#if totp_activated}
							<GreenButton on:click={disable_totp}
								>{$t('security_settings.disable_totp')}</GreenButton
							>
						{:else}
							<GreenButton on:click={enable_totp}
								>{$t('security_settings.enable_totp')}</GreenButton
							>
						{/if}
					</div>
				</div>
