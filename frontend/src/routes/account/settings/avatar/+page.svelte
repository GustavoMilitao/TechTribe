			...
			<div class="flex pl-2">
				<div class="mr-auto">
					<GreenButton
						...
						disabled={index < 1}>{$t('words.back')}</GreenButton
					>
				</div>
				<div class="mx-auto">
					<h2 class="text-2xl">
				...
				<div class="ml-auto">
					<GreenButton disabled={index < 11}>{$t('words.finish')}</GreenButton>
				</div>
			</div>
		...
		<div class="m-auto grid grid-cols-2 gap-4" in:fade={{ delay: 3500 }}>
			<GreenButton
				on:click={() => {
					index = 0;
					finished = false;
				}}>{$t('avatar_settings.start_over')}</GreenButton
			>
			<GreenButton on:click={save_avatar} flex={true} disabled={save_finished === true}>
			...
			<GreenButton href="/account/settings">{$t('avatar_settings.go_back')}</GreenButton>
			<GreenButton
				on:click={() => {
					finished = false;
				}}>{$t('words.close')}</GreenButton
			>
		</div>
	</div>
{/if}
