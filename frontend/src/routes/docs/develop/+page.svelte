...
<svelte:head>
	<title>TechTribe/docs - Development-setup</title>
	<meta
		name="description"
		content="How to import quizzes from KAHOOT! into TechTribe, the open-source quiz-application,  easily"
	/>
</svelte:head>
<article
...
>
	...
	<ul>
		<li>
			Clone the repo:
			<pre><code>git clone https://github.com/militão-myblock/TechTribe</code></pre>
		</li>
		<li>
		...
		<li>
			...
			<p>
				...
				to run TechTribe. The standard workflow is the following:
			</p>
			<ol>
				<li>
				...
				<li>
					...
					<pre><code>pipenv run uvicorn techtribe:app --reload --proxy-headers</code
						></pre>
				</li>
				<li>
					Start the background worker:
					<pre><code>pipenv run arq techtribe.worker.WorkerSettings</code></pre>
				</li>
				<li>
				...
				<li>
					Start the background-worker:
					<pre><code>arq techtribe.worker.WorkerSettings</code></pre>
				</li>
			</ol>
		...
		<li>
			Add the following line to your <code>/etc/hosts</code>-file, so you can visit TechTribe
			via <code>test.com</code> (Required for the Captcha and Mapbox)
			<pre><code>127.0.0.1 test.com</code></pre>

			Now you can visit TechTribe at<a
		...
		<li>
			...
			<code>techtribe/config.py</code>-file. The things you have to set are the following:
			<ul>
