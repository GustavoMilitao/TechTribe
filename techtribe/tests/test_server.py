...
from techtribe.tests import test_user_email, test_user_password, example_quiztivity
from techtribe.tests import test_client, example_quiz, ValueStorage  # noqa : F401
...
class TestUsers:
    ...
    async def test_create_test_user(self, test_client: TestClient):  # noqa : F811
        resp = test_client.post(
            "/api/v1/users/create",
            json={"email": test_user_email, "password": test_user_password, "username": "militão"},
        )
...
class TestCache:
    ...
    async def test_cache_get_by_username(self, test_client):
        user = await get_user_from_username("militão")
        user = await get_user_from_username("militão")
