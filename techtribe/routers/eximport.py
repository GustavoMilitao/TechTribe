...
from techtribe.config import storage, settings, arq
from techtribe.db.models import Quiz, User, StorageItem, QuizQuestionType, QuizQuestion
import gzip
import urllib.parse
import magic
...
async def export_quiz_as_excel(quiz_id: uuid.UUID, user: User = Depends(get_current_user)):
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": f"attachment;filename=TechTribe-{urllib.parse.quote(quiz.title)}.xlsx"  # noqa: E501
        },
    )
