...
from techtribe.db.models import User, GameResults
...
async def export_result(result_id: UUID, user: User = Depends(get_current_user)):
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": f"attachment;filename=TechTribe-{urllib.parse.quote(quiz.title)}-{datetime.strftime('%m-%d-%Y')}.xlsx"
            # noqa: E501
        },
    )
